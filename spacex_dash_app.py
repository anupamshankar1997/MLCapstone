...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        title = f'Total Successful Launches by Site'
        fig = px.pie(spacex_df, values='class', names = 'Launch Site', title=title)
        return fig
    else:
        drop = spacex_df[spacex_df['Launch Site']== entered_site ]
        launch = drop.groupby(['Launch Site','class']).size().reset_index(name='class count')
        title = f'Successful Launches for Site {entered_site}'
        fig = px.pie(launch, values='class count', names='class', title=title)
        return fig
